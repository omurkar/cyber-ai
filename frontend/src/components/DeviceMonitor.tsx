                ...
                <div className="grid md:grid-cols-2 gap-4">
                    <Card title="Overview">
                        <div className="text-sm space-y-1">
                            ...
                            <div>Suspicious processes: {result?.result?.suspicious_processes?.length ?? 0}</div>
                            <div>Open ports: {result?.result?.open_ports?.length ?? 0}</div>
                        </div>
                    </Card>
                    <Card title="Suspicious Processes">
                        <ul className="text-sm list-disc pl-5 space-y-1 max-h-48 overflow-auto">
                            {(result?.result?.suspicious_processes || []).map((p: any, i: number) => (
                                <li key={i}>{p?.name ?? 'unknown'} {p?.pid ? `(pid ${p.pid})` : ''}</li>
                            ))}
                        </ul>
                    </Card>
                    <Card title="Open Ports">
                        <ul className="text-sm grid grid-cols-2 gap-2 max-h-48 overflow-auto">
                            {(result?.result?.open_ports || []).map((prt: any, i: number) => (
                                <li key={i} className="px-2 py-1 bg-gray-900 rounded border border-gray-800">{prt}</li>
                            ))}
                        </ul>
                    </Card>
                </div>
